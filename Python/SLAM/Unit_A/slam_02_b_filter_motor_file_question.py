...
def filter_step(old_pose, motor_ticks, ticks_to_mm, robot_width,
    ...
    if motor_ticks[0] == motor_ticks[1]:
        ...
        robotx = old_pose[0] - scanner_displacement*cos(old_pose[2])
        roboty = old_pose[1] - scanner_displacement*sin(old_pose[2])
        alpha = (motor_ticks[1]-motor_ticks[0])*ticks_to_mm/robot_width
        R = motor_ticks[0]*ticks_to_mm/alpha
        cx = robotx - (R+robot_width/2.0)*sin(old_pose[2])
        cy = roboty - (R+robot_width/2.0)*(-cos(old_pose[2]))
