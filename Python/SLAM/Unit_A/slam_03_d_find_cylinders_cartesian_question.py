...
def find_cylinders(scan, scan_derivative, jump, min_dist):
    ...
    for i in xrange(len(scan_derivative)):
        ...
        if scan_derivative[i]>jump:
            if on_cylinder:
                location = (sum_ray/rays, sum_depth/rays)
                cylinder_list.append(location)
            on_cylinder = False
...
def compute_cartesian_coordinates(cylinders, cylinder_offset):
    ...
    for c in cylinders:
        ...
        modified_range = c[1]+cylinder_offset
        x = modified_range*cos(beam_angle)
        y = modified_range*sin(beam_angle)
        result.append((x,y))
